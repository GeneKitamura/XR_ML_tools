...
def count_dirs(img_root=IMG_ROOT, del_error_path=False):
    dir_count = 0
    multi_pt_list = []
    all_pt_list = []
    single_pt_list = []
...
def get_full_paths(df_based_on_files):
    ...
    for idx, row in df_based_on_files.iterrows():
        try:
            full_acc_path = os.path.join(IMG_ROOT, row['pat_path'], row['acc_path'])
            ...
            for series in os.listdir(full_acc_path):
                ...
                if len(dicoms) == 0:
                    empty_acc_id_tup_list.append(row['id_acc_tup'])
                    continue
                ...
                for dicom in dicoms:
                    ...
                    full_path_list.append(dicom_path)
    ...
    return full_path_df, empty_acc_id_tup_list
...
class Data_Fidelity():
    def __init__(self, map_file=MAP_FILE, img_root=IMG_ROOT, r3_req=R3_REQ, params=None):
